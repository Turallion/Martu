...
export default function AnniversarySite() {
  ...
  return <main className="relative min-h-screen overflow-hidden">
    ...
    <AnimatePresence mode="wait">
      {screen === 'intro' && <Frame key="intro"><motion.p initial={{opacity:0}} animate={{opacity:1}} className="mb-4 text-sm font-semibold uppercase tracking-[.35em] text-pink-500/80">Happy anniversary!</motion.p><h1 className="mb-7 text-4xl font-black sm:text-6xl">I made something for you ❤️</h1><RomanticImage src={img('intro-main')} alt="intro" /><p className="mt-7 text-lg font-bold text-pink-700/80 sm:text-xl">curious about what's in there?</p><div className="mt-5 flex gap-4"><Button onClick={() => setScreen('right')}>Yes</Button><Button onClick={() => setScreen('no')}>No</Button></div></Frame>}
